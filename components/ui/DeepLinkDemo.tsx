...
export const DeepLinkDemo: React.FC = () => {
  ...
  const handleTestDeepLink = (type: 'sermon' | 'article' | 'category', id: string, label: string) => {
    ...
    <Card style={{ margin: 16, backgroundColor: theme.colors.surface }}>
      <Card.Content>
        ...
        <Text variant="bodyMedium" style={{ marginBottom: 16, color: theme.colors.onSurfaceVariant }}>
          Test the deep linking functionality by generating sample links for different content types.
        </Text>
        ...
        <Text variant="bodySmall" style={{ marginTop: 16, color: theme.colors.onSurfaceVariant, textAlign: 'center' }}>
          These deep links can be shared via messaging, email, or social media. When opened, they will take users directly to the specific content in the TRUEVINE FELLOWSHIP app.
        </Text>
      </Card.Content>
    </Card>
  );
